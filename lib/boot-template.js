...
const loadDependency = async (bundleName, graph) => {
  const bundle=graph.assets[bundleName];
  if(!bundle){
    throw new Error (`unable to resolve bundle : ${bundleName} `);
  }
  const dependencies = bundle.js;
